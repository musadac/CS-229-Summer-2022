...
def forward_prop(data, labels, params):
    ...
    # *** START CODE HERE ***
    epi = 1e-4
    activation = sigmoid(np.matmul(data, params["W1"]) + params["b1"])
    out = softmax(np.matmul(activation, params["W2"]) + params["b2"])
    loss = - np.multiply(labels, np.log(out + epi))
    loss = loss.sum() / data.shape[0]
    return activation, out, loss
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    acti, out, loss = forward_prop_func(data, labels, params)
    delta = (out - labels).reshape(data.shape[0], labels.shape[1])
    delta1 = np.matmul(delta , params['W2'].T) * (acti * (1 - acti))
    dW = np.matmul(acti.T, delta)
    dW1 = np.matmul(data.T, delta1)
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    """
    # *** START CODE HERE ***
    unreg = backward_prop(data, labels, params, forward_prop_func)
    grad = {}
    grad["W1"] = unreg["W1"] + (2 * reg) * params["W1"]
    grad["W2"] = unreg["W2"] + (2 * reg) * params["W2"]
    grad["b1"] = unreg["b1"]
    grad["b2"] = unreg["b2"]
    return grad
    # *** END CODE HERE ***
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    # *** START CODE HERE ***
    split = np.linspace(train_data.shape[0], batch_size).astype(int)[::-1]
    for i in split:
        x = train_data[i-batch_size:i, :]
        y = train_labels[i-batch_size:i, :]
        grad = backward_prop_func(x, y, params, forward_prop_func)
        for var in params.keys():
            params[var] -= learning_rate * grad[var]
    # *** END CODE HERE ***
