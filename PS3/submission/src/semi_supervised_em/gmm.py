...
def main(is_semi_supervised, trial_num):
    ...
    group = np.random.choice(K, x.shape[0])
    ...
    w = np.full((x.shape[0], K), fill_value=(1. / K), dtype=np.float32)
...
def run_em(x, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        it += 1
        print('iter :'+str(it)+ ' loglikelihood: '+str(ll))
        # *** END CODE HERE ***
...
def p_x_given_z(x, mu, sigma):
    ...
    x_minus_mu = x - mu
    c = 1 / ((2 * np.pi) ** (len(x) / 2) * np.sqrt(np.linalg.det(sigma)))
...
def m_step_ss(x, x_tilde, z_tilde, w, phi, mu, sigma, alpha):
    ...
    k_counts = [np.sum(z_tilde == j) for j in range(len(mu))]
    for j in range(len(mu)):
        phi[j] = (w_colsums[j] + alpha * k_counts[j]) / (x.shape[0] + alpha * x_tilde.shape[0])
        w_j = w[:, j:j + 1]
        mu[j] = ((np.sum(w_j * x, axis=0)+ alpha * np.sum(x_tilde[(z_tilde == j).squeeze(), :], axis=0))/ (np.sum(w_j) + alpha * k_counts[j]))
        sigma[j] = np.zeros_like(sigma[j])
        for i in range(x.shape[0]):
            x_minus_mu = x[i] - mu[j]
            sigma[j] += w[i, j] * np.outer(x_minus_mu, x_minus_mu)
