...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    s1 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
    s2 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])
    if s2 < s1:
        return 0
    elif s2 > s1:
        return 1
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for i in [0, 1]:
        for state in range(mdp_data['num_states']):
            num_transitions = np.sum(mdp_data['transition_counts'][state, :, i])
            if (num_transitions > 0):
                mdp_data['transition_probs'][state, :, i] = mdp_data['transition_counts'][state, :, i] / num_transitions
    for state in range(mdp_data['num_states']):
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    iter = 0
    while True:
        new_v = np.zeros(mdp_data['num_states'])
        for state in range(mdp_data['num_states']):
            v1 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
            v2 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])
            new_v[state] = max(v1, v2)
        new_v =  mdp_data['reward'] + gamma * new_v
        max_diff = float('-Inf')
        for state in range(mdp_data['num_states']):
            if(abs(new_v[state] - mdp_data['value'][state]) > max_diff):
                max_diff = abs(new_v[state] - mdp_data['value'][state])
